...
if __name__ == '__main__':
	...
	svhn_testdata = ImageDataset("../../hw3_data/digits/svhn/test/", \
		"../../hw3_data/digits/svhn/test.csv", \
		transform )
	svhn_test_dataloader = DataLoader(svhn_testdata, batch_size=128, shuffle=False, num_workers=2, drop_last=False)

	model = GTA()

	# model.feat_extracter.apply(weights_init_xavier)
	# model.source_classifier.apply(weights_init_xavier)
	# model.generater.apply(weights_init_xavier)
	# model.discriminator.apply(weights_init_xavier)
	...
	save_dir = "../../model/ImprovedModel/MnistM2Svhn/MnistM/"
	...
	for ep in range(epochs):
		...
		print("Evaluating ...")
		valid_ac = model_evaluate(model.feat_extracter, model.source_classifier, svhn_test_dataloader)
