...
if __name__ == '__main__':
	...
	svhn_testdata = ImageDataset("../../hw3_data/digits/svhn/test/", \
		...
		transform )
	svhn_test_dataloader = DataLoader(svhn_testdata, batch_size=128, shuffle=False, num_workers=2)
	...
	save_dir = "../../model/DaNN_Mnistm2Svhn/MnistM2Svhn/"
	...
	for ep in range(epochs):
		...
		Lambda = torch.FloatTensor([2 / (1+np.exp(-100 * p)) - 1]).cuda()

		print("Training ...")
		model_train(model, model_optim, mnist_dataloader, svhn_dataloader, Lambda)

		print("Evaluating ...")
		valid_ac = model_evaluate(model, svhn_test_dataloader)
